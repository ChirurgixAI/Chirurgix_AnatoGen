...
def bin_and_classify_3d_image(image, num_bins):
    ...
    bin_indices = np.digitize(flattened_image, bin_edges)
    bin_indices = bin_indices - 1
    bin_indices = np.clip(bin_indices, 0, None)


    binned_image = bin_indices.reshape(image.shape)
...
def image_to_point_cloud(image, z_scale, centroids_ratio=10000000):
    ...
    return np.array(non_zero_points), centroids
...
def main_show_image(path):
    ...
    return slices, image_mold, image_cancer, output_dir, plt_output_path_lst, app_type
...
class MyWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # 0820 
        # =====================================================
        # Add "Import Applicator Path" button
        btnImportApplicator = QPushButton(text="Import Applicator Path")
        btnImportApplicator.setMaximumWidth(150)  # Set maximum width
        btnImportApplicator.setFixedHeight(90)  # Set fixed height
        btnImportApplicator.clicked.connect(self.importAPPPath)  # Connect to appropriate function
        buttonsLayout.addWidget(btnImportApplicator)

        # Add text field for Applicator path
        self.appPathEdit = QLineEdit()
        self.appPathEdit.setFixedWidth(150)
        buttonsLayout.addWidget(self.appPathEdit)
        # ======================================================
        ...
        self.imageLayout = QVBoxLayout()  # Changed from QGridLayout() to QVBoxLayout()
        viewLayout.addLayout(self.imageLayout)
        self.images = [QGraphicsView()]  # Reduced number of QGraphicsView objects to 1
        self.images[0].setScene(QGraphicsScene())  # Set a new QGraphicsScene
        self.imageLayout.addWidget(self.images[0])  # Add the single image view to layout
    ...
    def importCTPath(self):
        ...
        self.slices, self.image_mold, self.image_cancer, self.output_dir, self.plt_output_path_lst, self.app_type = main_show_image(self.lastDir)
        self.showImage(self.plt_output_path_lst[0])

        # image, labels, colors, slices = read_ct(foldername)

    def importAPPPath(self):
        directory = str(Path.home())  # default to user's home directory
        if self.lastDir:
            directory = self.lastDir
        foldername = QFileDialog.getExistingDirectory(self, "Open folder", directory)
        if foldername:
            self.appPathEdit.setText(foldername)
            self.lastDir = foldername
            print(foldername)

        self.app_path = get_app_path(self.appPathEdit, self.app_type)

    def generateAndPreviewCT(self):
        ctPath = self.ctPathEdit.text()
        print(f"Processing CT file at {ctPath}")
        self.stlPath1, combined_mesh_with_lines, lines_mesh_combined, self.lines = main_genete_stl(self.slices,
                                                    self.image_mold,
                                                    self.image_cancer,
                                                    self.output_dir,
                                                    )
        # Add your own function here to process the CT file, generate STL and preview it
        # self.showSTL(self.stlPath1, 0)

        self.appPathmold, self.appPath = main_genete_app_stl(self.app_path,
                                            self.lines,
                                            self.output_dir,
                                            combined_mesh_with_lines, 
                                            lines_mesh_combined
        ...
        self.showSTL(self.appPathmold, 0)
    ...
    def showSTL(self, filename, viewer_index):
        ...
        distance = 200  
        self.viewer[viewer_index].setCameraPosition(distance=distance)        
    ...
    def showImage(self, imagePaths):
        ...
        image = QPixmap(imagePaths)
        self.images[0].scene().clear()
        self.images[0].scene().addPixmap(image)
        self.images[0].show()
