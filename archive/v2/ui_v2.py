...
def create_lines_from_centroids(centroids, image_mold, image_cancer, z_scale, z_extension=0.1):
    lines = []

    z_max = find_largest_non_zero_slice_z(image_mold) + 3
    
    
    for centroid in centroids:
        centroid_x, centroid_y, centroid_z = centroid
        new_z = z_max + z_scale * z_extension
        end_point = np.array([centroid_x, centroid_y, new_z])
        lines.append((centroid, end_point))
...
def add_lines_to_mesh(mesh, lines):
    ...
    for line in lines:
        ...
        cylinder = o3d.geometry.TriangleMesh.create_cylinder(radius=1, height=1.0)
        ...
        ortho_vec = find_orthogonal_vector(direction)
        rot_matrix = np.column_stack((ortho_vec, np.cross(direction, ortho_vec), direction))
        ...
        translation_matrix[:-1, -1] = start_point - (cylinder.get_center() - 0.5 * height * direction)
